...
#
# mail = Mail(app)
# ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg', 'gif', 'pdf', 'doc', 'docx', 'zip', 'avif', 'ico', 'heic', 'webp', 'txt', 'xlsx', 'csv', 'pptx', 'docx'}
# INVALID_MAILS_PATH = "Invalid_mails.csv"
# REPORTS_DIR = "reports"
# os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
# os.makedirs(REPORTS_DIR, exist_ok=True)
...
#         return set()
#     with open(INVALID_MAILS_PATH, 'r', encoding='utf-8') as f:
#         return set(line.split(',')[1].strip() for line in f if line.strip())
#
# def save_invalid_email(email, reason):
#     existing = load_invalid_emails()
#     if email in existing:
#         return
#     now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#     reason_code = 'format_error' if 'format' in reason.lower() else 'domain_error'
#     write_header = not os.path.exists(INVALID_MAILS_PATH) or os.stat(INVALID_MAILS_PATH).st_size == 0
#     with open(INVALID_MAILS_PATH, 'a', encoding='utf-8') as f:
#         if write_header:
#             f.write("Date-Time,To,Status,Failed Reason,Time Taken,Reason Code\n")
#         f.write(f"{now},{email},Failed,{reason},0,{reason_code}\n")
#
#
# def remove_invalid_email(email):
#     if not os.path.exists(INVALID_MAILS_PATH): return
#     with open(INVALID_MAILS_PATH, 'r', encoding='utf-8') as f:
#         lines = [line for line in f if email not in line]
#     with open(INVALID_MAILS_PATH, 'w', encoding='utf-8') as f:
#         f.writelines(lines)
...
#         if write_header:
#             f.write("Date-Time,Send mail address,To,cc,subject,message,status,failed reason,time taken\n")
#         preview = strip_tags(html_message).replace('\n', ' ').strip().replace(',', ';')[:200]
#         to_field = to if to else ''
#         cc_field = cc if cc else ''
#         f.write(f"{timestamp},{sender},{to_field},{cc_field},{subject},{preview},{status},{reason},{taken:.2f}s\n")
#
#
# def get_emails_from_csv(path):
#     with open(path, newline='', encoding='utf-8') as f:
...
#                         msg = Message(
#                             subject=final_subject,
#                             recipients=valid_to or [],  # either To or []
#                             cc=valid_cc or [],
#                             sender=from_display
#                         )
#
...
#         return jsonify({'error': 'Invalid file type'}), 400
#     fname = f"{uuid.uuid4().hex}_{secure_filename(uploaded_file.filename)}"
#     filepath = os.path.join(app.config['UPLOAD_FOLDER'], fname)
